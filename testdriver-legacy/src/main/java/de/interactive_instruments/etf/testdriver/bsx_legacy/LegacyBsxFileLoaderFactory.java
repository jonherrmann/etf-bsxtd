...
import static de.interactive_instruments.etf.testdriver.bsx_legacy.LegacyTypes.TEST_ITEM_TYPES;
 ...
 */
public class LegacyBsxFileLoaderFactory extends AbstractItemFileLoaderFactory<ExecutableTestSuiteDto>
     ...
     * Default constructor.
     */
    public LegacyBsxFileLoaderFactory(final DataStorage dataStorageCallback) {
    ...
    public void init()
        ...
        if ("equals".equals(this.configProperties.getProperty("etf.testdrivers.bsx.legacy.standalone"))) {
    ...
    public boolean couldHandle(final Path path) {
        return path.toString().endsWith(LegacyBsxConstants.ETS_DEF_FILE_SUFFIX);
    }

    @Override
    public FileChangeListener load(final Path path) {
        return new LegacyBsxFileLoader(this, path,
