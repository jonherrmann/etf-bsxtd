 ...
 */
public class TopoX implements Serializable, QueryResource {
     ...
     *
     * Parsed boundary objects will mark overlapping boundary edges in the topology. An unmarked boundary edge is not
     * enclosed by a boundary object and can therefore be reported as an error.
     *
     * Requires an initialized topology object that has already captured topological information.
     *
     * @param topologyId
     ...
     */
    @Requires(Permission.CREATE)
    public int newEdgeValidatorEnclosingBoundaries(final int topologyId)
            throws BaseXException {
        if (topologyId < 0 || topologyId >= themes.size()) {
            throw new BaseXException("Unknown topology ID: " + String.valueOf(topologyId));
        }
        // handlerType 3: enclosingBoundaryHandler
        this.boundaries.add(new EdgeValidator(themes.get(topologyId), 3));
        return this.boundaries.size() - 1 + BOUNDARY_ID_OFFSET;
    }
    ...
    /**
     * Switch the Topology Builder and PosListParser to the next interior
     *
     * @param id
     *            ID of Topology Builder
     */
    ...
    public void nextInterior(final int id) {
        ...
        themes.get(id).parser.nextInterior();
    }
    ...
    @Deterministic
    @Requires(Permission.NONE)
    public int detectUnenclosedBoundaries(final int id) {
        return themes.get(id).detectUnenclosedBoundaries();
    }
