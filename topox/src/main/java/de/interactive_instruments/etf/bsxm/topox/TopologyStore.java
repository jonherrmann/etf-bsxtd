...
class TopologyStore implements Topology, TopologyMXBean {
    ...
    private final class FlyweightEdge implements Topology.Edge {
        ...
        public long rightObject() {
            ...
            if (right == Integer.MIN_VALUE || right == ENCLOSED_BOUNDARY_MARK_VALUE) {
                // check if this is a free-standing or enclosing surface mark
                return 0;
            }
            return right;
        }
    ...
    @Override
    public Iterable<Edge> unenclosedBoundaries() {

        // The edge index of the found unenclosed boundary edges
        final TIntArrayList foundUnenclosedBoundaryEdges = new TIntArrayList();

        builder.findUnenclosedBoundaries(foundUnenclosedBoundaryEdges);

        final int maxPos = foundUnenclosedBoundaryEdges.size();
        ...
        return () -> new Iterator<Edge>() {
            ...
            @Override
            public Edge next() {
                return new FlyweightEdge(foundUnenclosedBoundaryEdges.get(currentPos++));
            }
        };
    }
