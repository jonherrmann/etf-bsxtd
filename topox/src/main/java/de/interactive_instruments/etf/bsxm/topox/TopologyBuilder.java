...
public class TopologyBuilder implements HashingSegmentHandler {
    ...
    // Right side location value that marks an edge as an unenclosed boundary
    public static final int ENCLOSED_BOUNDARY_MARK_VALUE = Integer.MIN_VALUE + 1;
    ...
    /**
     * Find unenclosed boundary edges in the topological data structure
     *
     * @param foundUnenclosedBoundaryEdges
     *            array for adding the unenclosed boundary edges
     */
    void findUnenclosedBoundaries(final TIntArrayList foundUnenclosedBoundaryEdges) {
        final int maxEdgeSearch = 1_000_000;

        for (int i = TOPOLOGY_FIELDS_SIZE; i < topology.size(); i += TOPOLOGY_FIELDS_SIZE) {
            // Check if an object is set on the right side
            if (checkIfOutsideExteriorEdgeAddUnenclosedEdgeAndMark(i, foundUnenclosedBoundaryEdges)) {
                // Found the first exterior edge without anything on the right side.
                // Mark this edge, add it if the edge is enclosed and begin to iterate along the edges until we either
                // return to this edge or find an edge that has something on the right side.
                int next = getLeftOrRightByIndex(-i, CCWI_OFFSET);
                int steps = 0;
                for (; steps < maxEdgeSearch && i != next && -i != next &&
                        checkIfOutsideExteriorEdgeAddUnenclosedEdgeAndMark(next, foundUnenclosedBoundaryEdges); steps++) {
                    next = getLeftOrRightByIndex(-next, CCWI_OFFSET);
                }
            }
        }
    ...
    /**
     * Check if an edge is an outside exterior edge. Add the outside exterior edge to foundUnenclosedBoundaryEdges, if it is
     * not marked as being enclosed. Mark outside exterior edges as having been visited.
     *
     * @param edgeIndex
     *            edge index
     * @param foundUnenclosedBoundaryEdges
     *            array for adding found unenclosed boundary edges
     * @return true if edge is an exterior edge without an object on the right side
     */
    private boolean checkIfOutsideExteriorEdgeAddUnenclosedEdgeAndMark(final int edgeIndex,
            final TIntArrayList foundUnenclosedBoundaryEdges) {
        if ((edgeIndex > 0 && topology.getQuick(edgeIndex + RIGHT_LOCATION_INDEX) == 0) ||
                (edgeIndex < 0 && topology.getQuick(-edgeIndex + LEFT_LOCATION_INDEX) == 0)) {
            // The edge does not have anything on the right side and is not marked as enclosed or visited
            foundUnenclosedBoundaryEdges.add(edgeIndex);
        ...
        } else if ((edgeIndex > 0 && topology.getQuick(edgeIndex + RIGHT_LOCATION_INDEX) == ENCLOSED_BOUNDARY_MARK_VALUE) ||
                (edgeIndex < 0 && topology.getQuick(-edgeIndex + LEFT_LOCATION_INDEX) == ENCLOSED_BOUNDARY_MARK_VALUE)) {
            final long obj = topology.getQuick(edgeIndex + OBJ_OFFSET);
            // Check if this is an exterior edge
            if (getLeft(obj) > 0) {
                // The edge is marked as enclosed
                // Mark it as visited
                topology.setQuick(edgeIndex + RIGHT_LOCATION_INDEX, Integer.MIN_VALUE);
                return true;
            }
    ...
    /**
     * Mark the given edge as being enclosed by a boundary object.
     *
     * @param edgeIndex
     *            index of the edge to mark as being enclosed.
     */
    void markEnclosedEdge(final int edgeIndex) {
        topology.setQuick(abs(edgeIndex) + RIGHT_LOCATION_INDEX, ENCLOSED_BOUNDARY_MARK_VALUE);
    }
