...
(:~
 ...
 :
 : This function may only be called once. The result should be cached by the caller.
 : This function may not be used together with the enclosing-boundary validator
 : and the detect-unenclosed-boundaries() function.
 :
 : Throws BaseXException if the $topologyId is unknown
 :
...
(:~
 : Checks the topology for unenclosed boundaries.
 :
 : Must be called after the parse-surface() function.
 : Must be called after the parse-boundary() function, with an EdgeValidator initialized with the function
 : new-validator-enclosing-boundaries().
 : Errors of the type BOUNDARY_EDGE_UNENCLOSED can be retrieved by
 : calling the topological-errors() function.
 :
 : The error includes the source coordinate of the boundary edge in the topology, which is not enclosed by parsed boundary objects.
 : The error also includes the surface object, to which the boundary edge belongs to.
 :
 : This function may only be called once. The result should be cached by the caller.
 : This function may not be used together with the detect-free-standing-surfaces() function.
 :
 : Throws BaseXException if the $topologyId is unknown
 :
 : @param   $topologyId ID of the topology to check
 : @returns the number of free-standing surfaces found or 0 if there is only one outer border
 :)
declare function topox:detect-unenclosed-boundaries($topologyId as xs:int) as xs:int {
        let $initTime := prof:current-ms()
        let $unenclosedBoundariesCount := java:detectUnenclosedBoundaries($topologyId)
        let $duration := prof:current-ms()-$initTime
        let $logDummy := prof:dump($unenclosedBoundariesCount || " unenclosed boundaries detected in " || $duration || "ms")
        return $unenclosedBoundariesCount
};
...
(:~
 : Creates a new object to check if boundary edges of the topology are being enclosed by boundary objects.
 :
 : Parsed boundary objects will mark overlapping boundary edges in the topology.
 : An unmarked boundary edge is not enclosed by a boundary object and can therefore be reported as an error.
 :
 : Requires an initialized topology object that has already
 : captured topological information with the parse-surface() function.
 : Errors can be retrieved by calling the appropriate detect-errors() function and by calling the topological-errors()
 : function.
 :
 : Throws BaseXException if the topologyId is unknown
 :
 ...
 : @return ID of the boundary check object
 :)
declare function topox:new-validator-enclosing-boundaries($topologyId as xs:int) as xs:int {
    java:newEdgeValidatorEnclosingBoundaries($topologyId)
};
