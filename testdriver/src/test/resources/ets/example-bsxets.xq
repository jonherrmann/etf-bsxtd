...
for $i in 0 to $count return if (db:exists($dbBaseName || '-000')) then () else error($paramerror,concat("System error: XML database '",concat($dbBaseName,"-",$i),"' was specified, but not found. Please contact an administrator.&#xa;")),
...
let $db := db:open($dbBaseName || '-000')[matches(db:path(.),$files_to_test)]
